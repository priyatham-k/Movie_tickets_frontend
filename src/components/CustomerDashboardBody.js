...
const CustomerDashboardBody = () => {
  ...
  const fetchMovies = async () => {
    try {
      ...
      const initialStates = response.data.reduce((acc, movie) => {
        ...
        console.log(movie.screen?.capacity)
        return acc;
      }, {});
  ...
  const handleSeatSelection = (movieId, seat) => {
    setMovieStates((prev) => ({
      ...
      [movieId]: {
        ...prev[movieId],
        selectedSeats: prev[movieId].selectedSeats.some(
          (selectedSeat) => selectedSeat.seatNumber === seat.seatNumber
        )
          ? prev[movieId].selectedSeats.filter(
  ...
  const handleConfirmBooking = (movieId) => {
    const { selectedDate, selectedTimeSlot, selectedSeats } =
      movieStates[movieId];
  ...
  const handlePayment = async () => {
    const { movieId, cardHolder, creditCard, expiry, cvv } = paymentDetails;
    const { selectedDate, selectedTimeSlot, selectedSeats } =
      movieStates[movieId];
    ...
    if (!cardHolder.match(/^[A-Za-z\s]{2,50}$/)) {
      errors.cardHolder =
        "Cardholder name must be between 2 and 50 alphabetic characters";
    }
    if (!creditCard.match(/^\d{16}$/))
      errors.creditCard = "Credit card number must be 16 digits";
    if (!expiry.match(/^\d{2}\/\d{2}$/))
      errors.expiry = "Expiry date format should be MM/YY";
            ...
            <div className="col-12 mb-3" key={movie._id}>
              ...
              >
                <div className="row g-0">
                        ...
                        {getTodayAndTomorrow().map((date) => (
                          <button
                            ...
                            className={`btn btn-outline-secondary btn-sm ${
                              movieState.selectedDate === date ? "active" : ""
                            }`}
                        ...
                        {movie.timeSlots.map((slot) => (
                          <button
                            ...
                            className={`btn btn-outline-primary btn-sm ${
                              movieState.selectedTimeSlot === slot
                                ? "active"
                                : ""
                            }`}
                            onClick={() =>
                              handleTimeSlotSelection(movie._id, slot)
                            }
                          >
                            {slot}
                          </button>
                  ...
                  {movieState.isActive &&
                    movieState.selectedDate &&
                    movieState.selectedTimeSlot && (
                      <div style={{ padding: "10px" }}>
                        ...
                        >
                          {Array.from(
                            { length: movieState.seatCapacity },
                            (_, i) => i + 1
                          ).map((seat) => {
                            const seatNumber = `Seat${seat}`;
                            const seatData = movieState.bookedSeats.find(
                              (bookedSeat) =>
                            ...
                            const isPendingCancel =
                              seatData?.status === "Pending Cancel";
                            const isSelected = movieState.selectedSeats.some(
                              (selectedSeat) =>
                                selectedSeat.seatNumber === seatNumber
                            );

                            let buttonClass = "btn-outline-primary"; // Default style
                            if (isBooked) {
                              buttonClass = "btn-secondary disabled"; // Booked seat
                            } else if (isPendingCancel) {
                              buttonClass = "btn-warning"; // Pending cancel
                            } else if (isSelected) {
                              buttonClass = "btn-success"; // Selected seat
                            }
                            ...
                            return (
                              <button
                                ...
                                disabled={isBooked || isPendingCancel} // Disable if booked or pending cancel
                              >
                                {seat}
                              </button>
                            );
                        ...
                        <button
                          ...
                          onClick={() => handleConfirmBooking(movie._id)}
                        >
                          Book Now
                        </button>
                      </div>
        ...
        <Modal.Body>
          <div className="border p-3 rounded mb-3">
            ...
            <p className="mb-1">
              ...
              {movies.find((movie) => movie._id === paymentDetails.movieId)
                ?.title || "N/A"}
            </p>
            <p className="mb-1">
              <strong>Date:</strong>{" "}
            ...
            <p className="mb-1">
              ...
              {movieStates[paymentDetails.movieId]?.selectedSeats
          ...
          />
          {errors.creditCard && (
            <small className="text-danger">{errors.creditCard}</small>
          )}
